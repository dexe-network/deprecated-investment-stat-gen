...
async function getPairContractAddress(web3: Web3, state: IState, sendToken: string, receiveToken: string): Promise<string> {
    try {
        const contract = new state.web3.eth.Contract(state.abis.abiPancakeFactory, state.baseAddresses.pancakeFactory);
...
async function getTokensData(web3: Web3, state: IState, pairAddress: string, sendToken: string, receiveToken: string, reserves: IReserveData): Promise<ITokenPriceData> {
    try {
        const contract = new state.web3.eth.Contract(state.abis.abiPairContract, pairAddress);
...
export async function getCurrentExchangeRate(web3: Web3, state: IState, sendToken: string, receiveToken: string): Promise<string> {
